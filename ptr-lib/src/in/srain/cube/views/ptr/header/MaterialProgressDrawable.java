 ...
 *  zzj更改了setupAnimators和start方法中的bug
 * @hide
 */
public class MaterialProgressDrawable extends Drawable implements Animatable {
    ...
    private static final Interpolator LINEAR_INTERPOLATOR = new LinearInterpolator();//以常量速率改变
    private static final Interpolator END_CURVE_INTERPOLATOR = new EndCurveInterpolator();
    private static final Interpolator START_CURVE_INTERPOLATOR = new StartCurveInterpolator();
    private static final Interpolator EASE_INTERPOLATOR = new AccelerateDecelerateInterpolator();//在动画开始和结束的地方比较快，中间较慢
    // Maps to ProgressBar default style
    ...
    public void start() {
        mAnimation.reset();
        //mRing.storeOriginals();//zzj更改 保存现在的动画分支，现在有bug，这句注释掉，改为下面的resetOriginals，之后会走else语句
        mRing.resetOriginals();//zzj添加
        // Already showing some part of the ring
        if (mRing.getEndTrim() != mRing.getStartTrim()) {
            mParent.startAnimation(mFinishAnimation);
            Log.i("zzj", "Already showing some part of the ring");
        } else {
            Log.i("zzj", "NOT Already showing some part of the ring");
            mRing.setColorIndex(0);
            //mRing.resetOriginals();//zzj注释
            mRing.setShowArrow(false);//zzj添加
        ...
        final Animation finishRingAnimation = new Animation() {
            public void applyTransformation(float interpolatedTime, Transformation t) {
                ...
                final float startTrim = ring.getStartingStartTrim()
                        ...
                        * interpolatedTime;
                //-----------zzj添加 避免了在interpolatedTime为1的时候，ring头尾相重，重新画了个圈
                if (startTrim < ring.getEndTrim()) {
